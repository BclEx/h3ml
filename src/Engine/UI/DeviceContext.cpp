...
vec4 DeviceContext::colorPurple;
vec4 DeviceContext::colorOrange;
vec4 DeviceContext::colorYellow;
vec4 DeviceContext::colorGreen;
vec4 DeviceContext::colorBlue;
vec4 DeviceContext::colorRed;
vec4 DeviceContext::colorBlack;
vec4 DeviceContext::colorWhite;
vec4 DeviceContext::colorNone;
...
void DeviceContext::SetTransformInfo(const idVec3 &org, const idMat3 &m)
{
	origin = org;
	mat = m;
...
// 
void DeviceContext::GetTransformInfo(idVec3 &org, idMat3 &m)
{
	m = mat;
	org = origin;
}

void DeviceContext::EnableClipping(bool b)
{
	enableClipping = b;
};

void DeviceContext::PopClipRect()
{
	if (clipRects.Num())
		clipRects.RemoveIndex(clipRects.Num() - 1);
}

void DeviceContext::PushClipRect(Rectangle r)
{
	clipRects.Append(r);
}

bool DeviceContext::ClippedCoords(float *x, float *y, float *w, float *h, float *s1, float *t1, float *s2, float *t2)
{
	if (!enableClipping || clipRects.Num() == 0)
		return false;
	int c = clipRects.Num();
	while (--c > 0) {
		Rectangle *clipRect = &clipRects[c];
