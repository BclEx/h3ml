...
namespace litehtml
{
	...
	private:
		...
		int _cols_count;
		rows _cells;
		table_column::vector _columns;
		table_row::vector _rows;
	public:
		table_grid()
		{
			_rows_count	= 0;
		...
		table_column &column(int c)	{ return _columns[c]; }
		table_row &row(int r) { return _rows[r]; }

		int rows_count() { return _rows_count; }
		int cols_count() { return _cols_count; }
