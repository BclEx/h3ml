...
element::element(const std::shared_ptr<document> &doc) : _doc(doc)
{
	_box = nullptr;
	_skip = false;
}
...
web_color element::get_color(const tchar_t *prop_name, bool inherited, const web_color &def_color)
{
	...
	return (!clrstr ? def_color : web_color::fro_string(clrstr));
}
...
bool element::is_inline_box() const
{
	...
	return (d == display_inline || d == display_inline_block || d == display_inline_text);
}
...
{
	if (is_visible()) {
		int p_left = std::min(pos.left(), x + _pos.left() - _padding.left - _borders.left);
		int p_right = std::max(pos.right(), x + _pos.right() + _padding.left + _borders.left);
		int p_top = std::min(pos.top(), y + _pos.top() - _padding.top - _borders.top);
		int p_bottom = std::max(pos.bottom(), y + _pos.bottom() + _padding.bottom + _borders.bottom);
		pos.x = p_left;
		pos.y = p_top;
...
element::ptr element::get_element_by_point(int x, int y, int client_x, int client_y)	LITEHTML_RETURN_FUNC(nullptr);
element::ptr element::get_child_by_point(int x, int y, int client_x, int client_y, draw_flag flag, int zindex) LITEHTML_RETURN_FUNC(nullptr);
...
element::ptr element::select_one(const tstring &selector)								LITEHTML_RETURN_FUNC(nullptr);
element::ptr element::find_adjacent_sibling(const element::ptr &el, const css_selector &selector, bool apply_pseudo /*= true*/, bool *is_pseudo /*= nullptr*/) LITEHTML_RETURN_FUNC(0);
element::ptr element::find_sibling(const element::ptr &el, const css_selector &selector, bool apply_pseudo /*= true*/, bool *is_pseudo /*= nullptr*/) LITEHTML_RETURN_FUNC(0);
...
void element::apply_stylesheet(const css &stylesheet)									LITEHTML_EMPTY_FUNC;
...
const tchar_t* element::get_attr(const tchar_t *name, const tchar_t *def /*= nullptr*/) LITEHTML_RETURN_FUNC(def);
...
const tchar_t *element::get_cursor()													LITEHTML_RETURN_FUNC(nullptr);
...
const tchar_t* element::get_style_property(const tchar_t *name, bool inherited, const tchar_t *def /*= nullptr*/) LITEHTML_RETURN_FUNC(0);
uint_ptr element::get_font(font_metrics *fm /*= nullptr*/)								LITEHTML_RETURN_FUNC(0);
