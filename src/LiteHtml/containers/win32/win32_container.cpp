...
{
	if (_hClipRgn)
		DeleteObject(_hClipRgn);
}

uint_ptr win32_container::create_font(const wchar_t* faceName, int size, int weight, font_style italic, unsigned int decoration)
{
	string_vector fonts;
	...
	lf.lfHeight = -size;
	lf.lfWeight = weight;
	lf.lfItalic = (italic == fontStyleItalic) ? TRUE : FALSE;
...
int win32_container::line_height(uint_ptr hdc, uint_ptr hFont)
{
...
int win32_container::text_width(uint_ptr hdc, const wchar_t* text, uint_ptr hFont)
{
	HFONT oldFont = (HFONT)SelectObject((HDC)hdc, (HFONT)hFont);
...
void win32_container::draw_text(uint_ptr hdc, const wchar_t* text, uint_ptr hFont, web_color color, const position& pos)
{
	apply_clip((HDC)hdc);
...
void win32_container::fill_rect(uint_ptr hdc, const position& pos, const web_color color, const css_border_radius& radius)
{
	apply_clip((HDC)hdc);
...
uint_ptr win32_container::get_temp_dc()
{
	return (uint_ptr) GetDC(NULL);
}

void win32_container::release_temp_dc(uint_ptr hdc)
{
	ReleaseDC(NULL, (HDC)hdc);
}

int win32_container::pt_to_px(int pt)
{
	HDC dc = GetDC(NULL);
...
int win32_container::get_text_base_line(uint_ptr hdc, uint_ptr hFont)
{
	HDC dc = (HDC)hdc;
	if (!dc)
	{
...
void win32_container::draw_list_marker(uint_ptr hdc, list_style_type marker_type, int x, int y, int height, const web_color& color)
{
	apply_clip((HDC)hdc);
...
void win32_container::load_image(const wchar_t* src, const wchar_t* baseurl, bool redraw_on_ready)
{
	std::wstring url;
...
void win32_container::get_image_size(const wchar_t* src, const wchar_t* baseurl, size& sz)
{
	std::wstring url;
	make_url(src, baseurl, url);
...
void win32_container::draw_image(uint_ptr hdc, const wchar_t* src, const wchar_t* baseurl, const position& pos)
{
	apply_clip((HDC)hdc);
...
void win32_container::draw_background(uint_ptr hdc, const wchar_t* image, const wchar_t* baseurl, const position& draw_pos, const css_position& bg_pos, background_repeat repeat, background_attachment attachment)
{
	apply_clip((HDC)hdc);
...
wchar_t win32_container::toupper(const wchar_t c)
{
	return (wchar_t)CharUpper((LPWSTR)c);
}

wchar_t win32_container::tolower(const wchar_t c)
{
	return (wchar_t)CharLower((LPWSTR)c);
}

void win32_container::set_clip(const position& pos, bool valid_x, bool valid_y)
{
