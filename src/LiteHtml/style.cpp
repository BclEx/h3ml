...
string_map style::_valid_values; // = {
...
{
	for (props_map::const_iterator i = src._properties.begin(); i != src._properties.end(); i++)
		add_parsed_property(i->first.c_str(), i->second._value.c_str(), i->second._important);
}
...
void style::add_property(const tchar_t *name, const tchar_t *val, const tchar_t *baseurl, bool important)
{
	...
	else if (!t_strcmp(name, _t("border-spacing"))) {
	...
	else if (!t_strcmp(name, _t("border"))) {
		string_vector tokens;
	...
	else if (!t_strcmp(name, _t("border-bottom-left-radius"))) {
		string_vector tokens;
		split_string(val, tokens, _t(" "));
		if (tokens.size() >= 2) {
...
void style::parse_short_background(const tstring &val, const tchar_t *baseurl, bool important)
{
	...
	for (string_vector::iterator tok = tokens.begin(); tok != tokens.end(); tok++) {
		...
		else if (value_in_list(tok->c_str(), _t("left;right;top;bottom;center")) || iswdigit((*tok)[0]) || (*tok)[0] == _t('-')	|| (*tok)[0] == _t('.')	|| (*tok)[0] == _t('+')) {
			if (_properties.find(_t("background-position")) != _properties.end())
				_properties[_t("background-position")]._value = _properties[_t("background-position")]._value + _t(" ") + *tok;
			else
...
{
	bool is_valid = true;
	string_map::iterator vals = _valid_values.find(name);
	if (vals != _valid_values.end())
		if (!value_in_list(val, vals->second))
			is_valid = false;
	if (is_valid) {
		props_map::iterator prop = _properties.find(name);
		if (prop != _properties.end()) {
			if (!prop->second._important || (important && prop->second._important)) {
				prop->second._value = val;
				prop->second._important = important;
			}
		}
		else
			_properties[name] = property_value(val.c_str(), important);
	}
}
