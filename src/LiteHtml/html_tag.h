...
namespace litehtml
{
	...
	protected:
		box::vector _boxes;
		string_vector _class_values;
		tstring _tag;
		style _style;
		string_map _attrs;
		vertical_align _vertical_align;
		text_align _text_align;
		style_display _display;
		list_style_type _list_style_type;
		list_style_position _list_style_position;
		white_space _white_space;
		element_float _float;
		element_clear _clear;
		floated_box::vector _floats_left;
		floated_box::vector _floats_right;
		elements_vector _positioned;
		background _bg;
		element_position _el_position;
		int _line_height;
		bool _lh_predefined;
		string_vector _pseudo_classes;
		used_selector::vector _used_styles;		

		uint_ptr _font;
		int _font_size;
		font_metrics _font_metrics;

		css_margins _css_margins;
		css_margins _css_padding;
		css_borders _css_borders;
		css_length _css_width;
		css_length _css_height;
		css_length _css_min_width;
		css_length _css_min_height;
		css_length _css_max_width;
		css_length _css_max_height;
		css_offsets _css_offsets;
		css_length _css_text_indent;

		overflow _overflow;
		visibility _visibility;
		int _z_index;
		box_sizing _box_sizing;

		int_int_cache _cahe_line_left;
		int_int_cache _cahe_line_right;

		// data for table rendering
		std::unique_ptr<table_grid>	_grid;
		css_length _css_border_spacing_x;
		css_length _css_border_spacing_y;
		int _border_spacing_x;
		int _border_spacing_y;
		border_collapse _border_collapse;

		virtual void select_all(const css_selector &selector, elements_vector &res);

	public:
		html_tag(const std::shared_ptr<document> &doc);
		virtual ~html_tag();
		...
		virtual const tchar_t *get_attr(const tchar_t *name, const tchar_t *def = nullptr) override;
		...
		virtual const tchar_t *get_style_property(const tchar_t *name, bool inherited, const tchar_t *def = nullptr) override;
		...
		virtual element::ptr find_ancestor(const css_selector &selector, bool apply_pseudo = true, bool *is_pseudo = nullptr) override;
		virtual element::ptr find_adjacent_sibling(const element::ptr &el, const css_selector &selector, bool apply_pseudo = true, bool *is_pseudo = nullptr) override;
		virtual element::ptr find_sibling(const element::ptr &el, const css_selector &selector, bool apply_pseudo = true, bool *is_pseudo = nullptr) override;
	...
	protected:
		...
		element::ptr get_element_before();
		element::ptr get_element_after();
	};
	...
	inline elements_vector &html_tag::children()
	{
		return _children;
	}
}
