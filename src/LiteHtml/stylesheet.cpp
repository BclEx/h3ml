...
bool css::parse_selectors(const tstring &txt, const style::ptr &styles, const media_query_list::ptr &media)
{
...
{
	sort(_selectors.begin(), _selectors.end(), std::less<css_selector::ptr>());
}

void css::parse_atrule(const tstring &text, const tchar_t *baseurl, const std::shared_ptr<document> &doc, const media_query_list::ptr &media)
{
	if (text.substr(0, 7) == _t("@import")) {
		int sPos = 7;
		tstring iStr = text.substr(sPos);
				...
				if (doc_cont) {
					...
					if (!css_text.empty()) {
						...
						if (!tokens.empty())
						{
							...
							new_media = media_query_list::create_fro_string(media_str, doc);
							if (!new_media)
								new_media = media;
						}
		...
		if (b1 != tstring::npos) {
			...
			trim(media_type);
			media_query_list::ptr new_media = media_query_list::create_fro_string(media_type, doc);
			tstring media_style = (b2 != tstring::npos ? text.substr(b1 + 1, b2 - b1 - 1) : text.substr(b1 + 1));
