...
namespace litehtml
{
	...
	{
	public:
		virtual uint_ptr create_font(const tchar_t *faceName, int size, int weight, font_style italic, unsigned int decoration, font_metrics *fm) = 0;
		...
		virtual	void transfor_text(tstring &text, text_transform tt) = 0;
		...
		virtual std::shared_ptr<element> create_element(const tchar_t *tag_name, const string_map &attributes, const std::shared_ptr<document> &doc) = 0;
