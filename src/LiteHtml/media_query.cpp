...
media_query::ptr media_query::create_fro_string(const tstring &str, const std::shared_ptr<document> &doc)
{
		...
		if ((*tok) == _t("not"))
			...
			if (!expr_tokens.empty()) {
				...
				if (expr.feature != media_feature_none) {
					...
					query->_expressions.push_back(expr);
				}
			}
		}
		else
...
bool media_query::check(const media_features &features) const
{
	...
	if (_media_type == media_type_all || _media_type == features.type) {
		res = true;
		for (media_query_expression::vector::const_iterator expr = _expressions.begin(); expr != _expressions.end() && res; expr++)
...
media_query_list::ptr media_query_list::create_fro_string(const tstring &str, const std::shared_ptr<document> &doc)
{
	...
	for (string_vector::iterator tok = tokens.begin(); tok != tokens.end(); tok++) {
		trim(*tok);
		lcase(*tok);
		media_query::ptr query = media_query::create_fro_string(*tok, doc);
		if (query)
			list->_queries.push_back(query);
	}
	if (list->_queries.empty())
		list = 0;
	return list;
}
...
{
	bool apply = false;
	for (media_query::vector::iterator iter = _queries.begin(); iter != _queries.end() && !apply; iter++)
		if ((*iter)->check(features))
			apply = true;
	bool ret = (apply != _is_used);
	_is_used = apply;
	return ret;
}
